...
- **`app.py`** — the entire Flask application: one `app = Flask(__name__)` and all route definitions. Real routes (`/`, `/register`, `/login`, `/terms`, `/privacy`) render templates; placeholder routes (`/logout`, `/profile`, `/expenses/...`) currently return "coming in Step N" strings and are where feature work lands.
